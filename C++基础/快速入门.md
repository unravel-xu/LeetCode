...
| 迭代器类型                           | 迭代器说明                                                                                                                                                |
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| 输入迭代器`input iterator`           | 从容器中读取元素。输入迭代器只能一次读入一个元素向前移动，也就是单向遍历，并且只能被递增操作一次。这意味着，一旦通过输入迭代器读取了一个元素，就不能再次读取该元素。                                                                   |
| 输出迭代器`output iterator`          | 向容器中写入元素。输出迭代器与输入迭代器相反，它只提供了向容器中写入元素的能力。与输入迭代器一样，输出迭代器也只能保证单向遍历，并且只能被递增操作一次。                                                                         |
| 正向迭代器`forward iterator`         | 正向迭代器提供了更强大的功能，它支持读写操作、递增和递减操作。然而，正向迭代器仍然只能保证单向遍历。常见的使用前向迭代器的容器有`vector`、`list`、`deque`和`array`等。                                                    |
| 双向迭代器`bidirectional iterator`   | 双向迭代器在前向迭代器的基础上增加了对容器中元素的双向遍历能力，即支持向前和向后遍历。这使得我们可以更方便地操作容器中的元素。常见的使用双向迭代器的容器有`map`、`set`、`multimap`和`multiset`等。                                     |
| 随机访问迭代器`random-access iterator` | 随机访问迭代器是功能最强大的迭代器类型，它支持所有的迭代器操作，包括读写操作、递增、递减、比较和算术运算（如加法和减法）。这使得我们可以像操作数组一样操作容器中的元素，可向前向后跳过任意个元素。常见的使用随机访问迭代器的容器有`vector`、`deque`、`array`和`string`等。 |
...
```
`size` 函数返回 `vector` 的实际长度（包含的元素个数），`empty` 函数返回一个 `bool` 类型，表明 `vector` 是否为空。二者的时间复杂度都是 $O(1)$。所有的 STL 容器都支持这两个方法，含义也相同，之后我们就不再重复给出。`clear` 函数将 `vector` 清空。