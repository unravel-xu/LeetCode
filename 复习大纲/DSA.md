...
![|500](Images/Pasted%20image%2020241125201254.png)
假设 n-1 个节点的向量中，<font color="#ff0000">第一个失败情况</font>所对应的查找长度为 d(比较树左侧链的**权重和**)，则如图 n 个节点向量成功查找长度为 d+2(树边3->1，权值为 2)
$CT_{n-1}$ 成功总长度 $S\cdot(n-1)$，失败总长度 $F\cdot n$
$CT_n$ 成功总长度 $S\cdot(n-1)+2$，失败总长度 $F\cdot n+3$
