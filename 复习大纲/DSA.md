...
- Fibonacci 查找：版本 A 的基础上，当 $n=fib(k)-1$ 取 $mi = fib(k-1)-1$ (即不大于 n 的 fib)
...
![|400](Images/Pasted%20image%2020241217153241.png)
## 习题
...
3. 对于叶节点数量为 2018 的树，层序遍历使用的辅助队列规模绝对不超过 2018 (√)
...
8. 将以x 为根的子树节点个数记为x.size，如果将二叉树中满足x.size\*2<=x.parent.size 的<font color="#d83931">非根节点</font> x 称作“轻节点”，那么在包含 2024 个节点的二叉树中，一个节点至多可能有（ ）个轻的真祖先
   ![](Images/Pasted%20image%2020241202082105.png)
...
2. 有向无环图的拓扑排序是 DFS 的 ( )
   ...
   (D)回溯的逆序
   零出度算法使用 DFS，如下图
   ![](Images/Pasted%20image%2020241202092937.png)
...
![|450](Images/Pasted%20image%2020241130152949.png)
...
![](Images/Pasted%20image%2020241130154737.png)
...
例如插入 1，0，2
（判断）某伸展树一共有 n 个互异的关键码，x 是第二大的关键码，则在 find(x)的过程中可能出现 zig-zig 操作（×）
第二大关键码，至多出现一次 zig
## 10-B B-树
- B-树每一层代表一级存储结构
...
![](Images/Pasted%20image%2020241129200454.png)
...
黑高度 = 当前节点(包括)到外部节点(不包括)经过的黑节点数目
...
![|500](Images/Pasted%20image%2020241129215833.png)
...
（补充习题）包含 1500 个关键码的 16 阶 B 树，约定根节点常驻内存，且在各节点内部采用顺序查找，在单次成功查找的过程中，至多有多少个关键码需要和目标关键码比较？
...
- 平方试探：$[hash(key)+i^2]\%M$ 以平方数为距离，确定下一试探桶单元
	  优势：缓解数据聚集现象
	  缺陷：一定程度上破坏局部性
	  若 M 是素数（无所谓 4k+1 还是 4k+3），且装填因子≤0.5，只要有空桶就必然能找出来
...
![](Images/Pasted%20image%2020241130104037.png)
...
![](Images/Pasted%20image%2020241130120844.png)
...
![](Images/Pasted%20image%2020241130143808.png)
...
- 最差情况：$O(n\cdot m)$
【习题 11-9】假定所有字符 $s=|\Sigma|$ 出现的概率均等，在 P 和 T 的每一个对齐位置，需要<font color="#d83931">连续执行字符比对操作</font>的期望次数不超过 $\frac{s}{s-1}\leq2=O(1)$
## 13-C KMP
- 特别适用于顺序存储介质
- 单次匹配概率越大（字符集规模越小），优势越明显
【习题 11-4】在算法执行的整个过程中：
观察量 i 始终等于已经做过的成功比对（含与最左端虚拟通配符"\*"比对）次数
观察量 i - j 始终不小于已经做过的失败比对次数
## 13-D BM-BC
- BC：单次匹配概率越小，优势越明显
二维 bc 表：一张 $m\times|\Sigma|$ 的表格，其中 $bc[j]['X']$ 指向在 $P[j]$ <font color="#d83931">左侧</font>并与之最近的字符'X'
【习题 11-5】针对坏字符'X'在模式串 P 中位置太靠右，以至位移量为负的情况，是否能在 $P[j]$ 的左侧找到最靠右的字符'X'使得与坏字符对齐？
相当于用二维 bc 表替换一维 bc 表，但二维 bc 表如果能发挥作用，则当时的好后缀必然很长，此时用 gs 表完全可以替代 bc 表，所以不倾向于使用这种方法
## 13-E BM-GS
![](Images/Pasted%20image%2020241130163345.png)
b）使用 Galil 规则，最坏情况下也只要线性时间
![](Images/Pasted%20image%2020241130163845.png)
除了第一轮比对，在后续的各轮比对中，均只需比较模式串 P 中最靠右的 w 个字符，剩余 m-w 个字符必然匹配
【习题 11-10】BM 算法和 KMP 算法分别擅长于处理何种类型的字符串？
当<font color="#d83931">字符集规模较小</font>时，单次比对的成功概率较高，蛮力算法的效率较低。此时，KMP 算法稳定的线性复杂度，更能体现出优势；而采用 BC 表的BM 算法，却并不能大跨度地向前移动。
当<font color="#d83931">字符集规模较大</font>时，单次比对的成功概率较小，蛮力算法也能接近于线性的复杂度。此时，KMP 算法尽管依然保持线性复杂度，但相对而言的优势并不明显；而采用 BC 表的 BM 算法，则会因比对失败的概率增加，可以大跨度地向前移动。
习题11-9的意思是只要各字符等概率出现，BF 算法的效果很好，习题11-10 的意思是等概率的前提下，字符集越大 BF 算法就越好
![|500](Images/Pasted%20image%2020241204205736.png)
（判断）在对随机的二进制串做匹配时，采用 kmp 算法比仅采用 bc 表的 bm 算法通常效率更低（√）
随机的二进制串，所以 Pr=0.5，由图可知此时 kmp 效率低
（填空）设文本串 T，模式串 P 的长度分别为 2025 和 17 且 T 不含 P，只借助 BM 算法的 gs 策略，最好情况只需做多少次字符比对？
2025/17=119……2，只需 119 就行，多的 2 不必比对
## 13-F karp-Rabin
- 快速指纹计算
- kr算法在“各字符皆独立且均匀分布”的假定条件下，指纹相同的可能性应为1/M。其中 M 为散列表容量
## 13-G 键树(trie)
![](Images/Pasted%20image%2020241105193140.jpg)
## 习题
1. 在字符集各字符出现概率相同时，kmp 算法渐进时间复杂度接近蛮力算法.（√）
2. 未改进过的 next 表，KMP 算法可以在线性时间完成串匹配.(√)
3. KMP 的运行时间不随着单次匹配成功率的变化而变化，注意单次匹配成功率可以大于 0.5，例如0, 1 串 0 出现的概率是 0.9 而 1 出现的概率是 0.1，那么匹配成功的概率就是 0.9×0.9+0.1×0.1=0.82
4. 模式串和文本串均有 26 个大写字母组成，那么蛮力算法在最好情况下的时间复杂度<font color="#d83931">等于</font>KMP 算法，在平均情况下蛮力算法的时间复杂度<font color="#d83931">等于</font>KMP 算法
5. 对于长度为 m 的随机 0,1 串进行串匹配时，好后缀数组中 $gs[0]==1$ 的概率为
   >[!note]-
   >![](Images/Pasted%20image%2020241204215107.png)
   
6. 如果 bm 算法仅使用 gs 策略而不使用 bc 策略则不能保证在最好情况下有 O(n/m)的时间复杂度（n 为文本串长度，m 为模式串长度）(×)
   ![](Images/Pasted%20image%2020241204215241.png)
7. kr 算法在一次匹配指纹失败后，无法在 O(1)内找到下一个指纹(×)
8. 在 BM 算法中坏字符表的长度与模式串长度无关（√）
   只和字符集大小有关
# 14 排序
## 14-A 快速排序
...
## 习题
1. 折半插入算法在寻找插入的位置时采用二分查找，因此折半插入排序时间复杂度为 O(nlogn)（×）
   向量的话查找需要 O(logn)，但是移动需要 O(n)时间；列表的话，二分查找时间就已经是 O(n)
2. 基于 CBA 的排序算法对所有大小为 n 的数组进行排序的时间复杂度是Ω(nlgn)（×）
   插入和冒泡最好情况是 O(n)
3. 交换序列中的任一一对逆序元素，逆序对数必然减少. (√)
   >[!note]-
   >![](Images/Pasted%20image%2020241204220447.png)
   
4. 在插入排序过程中，有序前缀每增加一个元素，序列中的循环节个数即使不增加，亦不致减少 (×)
   >[!note]-
   >![](Images/Pasted%20image%2020241204220638.png)

5. 基于比较式算法可以在 O(n)内确定无序数据中的前 10%（√）
   k 选取是 O(n)的时间，然后再用 O(n)的时间确定前 10% 的数据
6. 考察对 QuickSort 算法的如下调整：任何子序列在规模缩减至 m 以下之后，便不再继续递归划分；待所有递归都已终止后，再对整个序列调用一次 InsertionSort，如果待排序序列总长为 n，那么 InsertionSort 需要花费多少时间？为什么？
   段间有序，段内无序，段长是 O(m)，每段的插入排序时间是 O(m²),一共是 O(n/m)段，所以总的时间复杂度的是 O(nm)，m 是常数级别的话，其实最后一趟插入排序的渐进时间复杂度就是 O(n)
7. 基数排序如果采用了不稳定的底层算法，则输出序列未必正确且不稳定. (√)