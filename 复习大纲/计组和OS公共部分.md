...
>[!note]-
>(1)题目很特别的是有一个 4B 大小的页目录，意味着页表项和页目录项不一定会存在页表中，虚拟页大小是 16Bytes，所以页内偏移是 4 位，虚拟地址空间是 256Bytes,所以页目录号+页号+页内偏移一共 log2256=8 位,页目录大小是 4Bytes,页目录项的大小是 1Byte,一个页目录表包含 4 个页目录项,所以页目录号一共 log24=2 位,页表号是 8−4−2=2 位
>
# IO
I/O 接口(I/O 控制器)的功能：
1. 数据缓冲
