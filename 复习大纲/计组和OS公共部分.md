...
- EPC(Exception Program Counter)异常程序计数器：异常/中断结束后从哪里开始重新执行
	  多周期 CPU：
		  ...
		  - 异常：指令执行错误后，EPC=上一条指令的地址（PC-4）
	 指令流水 CPU
		 *精确异常*
- BadVaddr Register：访存错误发生时虚拟地址
	  当内存访问出错时保护
	  可用于后续对 TLB 的维护
- 精确异常：在处理异常时，产生异常的位置之前的命令都执行完毕，异常受害指令和其之后的好像从来没有开始一样(异常结束后重新执行，这些指令的行为和异常没有发生时完全一样)，当异常是精确异常时，处理异常的软件就可以忽略 CPU 实现的时序影响。（RISC-V 要求）
MIPS 异常处理流程：
...
5. 由 X86 硬件完成的是？
   ...
   (B)形成中断入口地址 
   (C)Eax 寄存器保存
   注意 x86 和 MIPS 不同，获取中断源是由硬件完成，选A、B
6. switch_to 函数可以直接切换，意味着函数调用时不必保存断点 (×)
7. 若应用程序执行中外设产生中断，则操作系统接管 CPU 控制权并完成中断处理（√）
8. risc-v处理器在U态（用户态）下执行ecall指令会陷入到S态（内核态）中下列关于硬件会自动完成的操作中错误的是？
   A.保存进程的页表基址
   B.保存 ecall 执行指令的所在地址
   C.保存陷入原因“在U态执行ecall指令” 
   D.转跳到异常处理例程的入口地址
   A 是软件处理
9. 微内核操作系统的设备驱动程序大多运行在内核态（×）
10. 在缺页处理过程中，OS 可能包含的操作是？
    修改页表、磁盘 IO、分配页框、异常处理、发送信号
# RAID
...
| **I(Invalid)**   | 这行数据无效                                  |
![|300](Images/Pasted%20image%2020241215203217.png)
## Cache 缺失原因
1. 必然缺失
	- 开机或者是进程切换
	- 首次访问数据块
...
（判断）Cache 块大小会影响命中率，要使得 Cache-主存的平均访问时间最低，Cache 大小应该选择使得命中率最高的那个 Cache 块大小（×）
缺失率最低时，缺失损失最大，平均访问时间反而增加
## 提高命中率
...
>
>在第 2 轮循环，第 0~3 块根据 LRU 会替换第 0~3 组的第 1 路(因为此时第 0 路是第一轮循环中的块 64~67，是最后的块)，随后在访问 16~19 会因为刚刚的替换再次缺失，这时它们会替换第 0~3 组第 2 路的 32~35，这也使得访问 32~35 缺失，会替换第 0~3 组第 3 路的 48~51，访问 48~51 会替换第 0~3 组第 0 路的 64~67，最后访问 64~67 会替换第 0~3 组第 1 路的 0~3，循环结束，共计 20 次未命中，此后每轮循环相似，都只会发生在第 0~3 组
>
>所以命中率 p=(4352\*10-68(第一轮未命中)-9\*20)/43520 = 99.43%
>
>速度提高：假设访存要 10s，则 Cache 命中访问时间为 1s，Cache 不命中访问时间同访存，10/(1+(1-99.43%)\*10)=9.5 倍
# 内存管理
...
偏移量拼接在物理/虚拟页号后
![](Images/Pasted%20image%2020241208232610.png)
页表的性能问题：
- 访问一个内存单元需要 2 次内存访问
	- 第一次访问：获取页表项
	- 第二次访问：访问数据
...
[详解虚拟内存如何与物理内存进行映射](https://www.cnblogs.com/binlovetech/p/17571929.html "发布于 2023-07-21 16:54")
![|500](Images/Pasted%20image%2020241208224839.png)
注意：初始时，进程只有一张页目录表，页目录表里的 PDE 没有映射任何东西，这时进程需要访问一个物理内存页，而对物理内存页的映射任务主要是在一级页表的 PTE 中，所以首要的任务是建立一张一级页表，并用页目录表索引起来
![|500](Images/Pasted%20image%2020241208225237.png)
进程按需分配，只有当第一个一级页表已经映射满了，才会创建第二张表
#### 页寄存器
...
### PAE
[32 位的操作系统也能使用超过4G的内存-PAE 技术简介](https://www.addesp.com/archives/242)
PAE 技术可以使得<font color="#c0504d">多个进程</font>使用的总内存大于 4GB，但是单个进程的所使用的内存上限依然只有 4GB
将地址线由 32 根增加到 36 根，寻址空间就从 4GB 增加到了 64GB
````tab
tab: 未使用PAE
![](Images/Pasted%20image%2020241208203410.png)
1. 从 **CR3** 寄存器中取出**页目录**的基址。
2. 取出**线性地址**中的 22 位~31 位共 10 位作为**页目录项**在**页目录**中的索引，由此找到**页目录项**。
3. 根据**页目录项**中的内容中找到对应的**页表**的基址。
4. 取出**线性地址**中的 12 位~21 位共 10 位作为**页表项**在**页表**中的索引，由此找到**页表项**。
5. 取出**线性地址**中的 0 位~11 位共 12 位作为**页内偏移**，从**页表项**中取出**页基址**，两者相加则到**物理地址**
tab: 使用PAE
![](Images/Pasted%20image%2020241208203615.png)
1. 从 **CR3** 寄存器中取出**页目录指针表**的基址。
2. 取出**线性地址**的 30 位~31 位共 2 位作为**页目录指针**在**页目录指针表**中的索引，由此找到**页目录指针**。
3. 根据**页目录指针**获得对应的**页目录**的基址
4. 取出**线性地址**中的 21 位~29 位共 9 位作为**页目录项**在**页目录**中的索引，由此找到**页目录项**。
5. 根据**页目录项**找到对应**页表**的基址。
6. 取出**线性地址**中的 12 位~20 位共 9 位作为**页表项**在**页表**中索引，由此找到**页表项**。
7. 取出**线性地址**中的 0 位~11 位共 12 位作为**页内偏移**，从**页表项**中取出**页基**址，两者相加则到**物理地址**。
````
IntelX86-32CPU 使用分页管理，每页 4KB，逻辑地址格式如图，这种 CPU 最多支持 4GB 内存。为了使系统能够使用 64GB 内存，使用物理内存扩展技术，使物理地址长度变为 64 位，页面大小仍为 4KB。试设计逻辑地址格式，使其可以在支持物理内存扩展技术的 X86-32CPU 上运行。![](Images/Pasted%20image%2020241208203900.png)
原物理内存 4GB，每页 4KB，可分为 1M 个物理页=20 位。加上 12 位页内偏移，得到页表项大小 32 位。
新物理内存 64GB，每页 4KB，可分为 16M 个物理页=24 位。加上 12 位页内偏移，得到页表项大小 36 位。对齐要求，向上扩展取为 64 位=8B。则每个页只含 4KB/8B = 512 个页表项，可知页面 offset=12 位(不变)，二级页表 offset=9 位，一级页表 offset=9 位。页目录表剩 32-9-9-12 = 2 位
## TLB
TLB 使用关联存储实现，也是 Cache，和 CPU Cache 一样，TLB 也分为数据 TLB 和指令 TLB，也有多种映射方式，也存在多层级
TLB 缓存的是<font color="#ff0000">在 Cache 中的部分页组成的页表</font>
...
![|650](Images/Pasted%20image%2020241116172808.png)
![|650](Images/Pasted%20image%2020241116172722.png)
![|650](Images/07163541_gEDQ.webp)
进程切换对 TLB 的处理
<font color="#ff0000">在上下文切换时</font>，清空(flush) TLB，会导致新进程触发 TLB 未命中，如果频繁切换进程，开销会很高
...
（判断）进程虚拟地址空间的大小是由内存和硬盘的大小决定的（×）
是由 CPU 位数决定，而内存和硬盘只是起限制作用
...
- Clock 时钟置换算法
是 FIFO 和 LRU 的折中
在<font color="#c0504d">页表项</font>中增加访问位，各页面组织成环形链表，指针指向最先调入的页面
...
常见的页面置换算法中只有 LRU，OPT，不清空计数 LFU 不会产生 belady，其余的都会产生 belady 异常
- FIFO:
![](Images/Pasted%20image%2020241209184745.png)
- CLOCK，改进版 CLOCK(假设只访问不修改)
![](Images/Pasted%20image%2020241209184949.png)
- LFU(清空计数的)

| 访存  | 物理页 a  | 物理页 b  | 缺页  | 换出  |
| :-: | :----: | :----: | :-: | :-: |
|  0  | 0(1 次) |   -    |  0  |  -  |
|  0  | 0(2 次) |   -    |  -  |  -  |
|  1  | 0(2 次) | 1(1 次) |  1  |  -  |
...
13. x86-32 虚拟存储系统中,采用二级页表,一级页表可以不在内存中(×)
    一级页表得在内存中，二级页表可以不在
14. 分页内存管理下，每个进程都有自己的页表(√)
15. 当进程用完所有虚拟地址空间后,多级页表比一级页表所占的内存空间大（×）
    二级页表可以不在内存中，可能占用的内存空间更小
16. 通过页表机制可以实现进程间共享内存（√）
17. 在没有 MMU 的 CPU 上 Linux 内核可正常运行并具备基于页表机制的虚拟存储管理能力（×）
18. 在一个进程的页表中，页目录项和页表项不同时存在（×）
    有可能同时存在，页目录表和其中的一张二级页表重合，页目录项也是页表项
19. 有一台假想计算机，虚拟页和物理页帧大小均为 16Bytes，支持 256Bytes 的虚拟地址空间，有 256Bytes 的物理内存；采用二级页表，一个页目录项大小为 1Byte，一个页表项大小为 1Byte，1 个页目录大小为 4Bytes，页目录基址寄存器保存了页目录表的物理页帧号，占 1Byte
    ![](Images/Pasted%20image%2020241208214510.png)
    256Bytes 的整个物理内存内容如下：（其中数值都是十六进制）
    ![](Images/Pasted%20image%2020241208214558.png)
    (1)在这台假象计算机的虚拟地址中，页目录号，页表号和页内偏移各占多少位？
    (2)在这台假想的计算机上执行访存指令时，虚拟地址 0x32 经地址转换后得到的物理地址为多少？内容是什么？
    (3)已知这台假想的计算机上执行访存指令时，虚拟地址 0x12 经过地址转换后得到的物理地址为 0x12，请回答对应页目录号和内容，对应页表号和内容分别是什么?
>[!note]-
>(1)题目很特别的是有一个 4B 大小的页目录，意味着页表项和页目录项不一定会存在页表中，虚拟页大小是 16Bytes，所以页内偏移是 4 位，虚拟地址空间是 256Bytes，所以页目录号+页号+页内偏移一共 8 位，页目录大小是 4Bytes，页目录项的大小是 1Byte，一个页目录表包含 4 个页目录项，所以页目录号一共 2 位，页表号是 8−4−2 = 2 位
>
>(2) 0x32 对应二进制 $00_{PDE} 11_{PTE}$ 0010，页目录基址寄存器的内容是 0x00，偏移量为 PDE $00_{2进制}=0_{16进制}$ 
>![](Images/Pasted%20image%2020241208230150.png)
>而页目录项大小 1B 所以对应的页目录项为 0x0C(8 位)，也就是 0000 1100，存在位是 1![](Images/Pasted%20image%2020241208230802.png)
>页目录项前 4 位是二级页表基址，所以二级页表的基址是 0x0，而 PTE= $11_{2进制}=3_{16进制}$，二级页表项地址就是 0x03
>![](Images/Pasted%20image%2020241208230150.png)
>页表项大小同样是 1B，所以对应的页表项为 0x3C，也就是 0011 1100，存在位是 1，前四位是物理页基址 0011，页内偏移是 0010，最终的物理地址就是 0011 0010，转为 16 进制是 0x32，内容是 0xEC
>(3)同(2)

20. 假定一个计算机系统中有一个TLB和一个L1 Data Cache。该系统按字节编址，虚拟地址16位，物理地址12位，页大小128B，TLB为四路组相连，共有16个页表项，L1 Data Cache采用直接映射方式，块大小为4B，共16行。在系统运行到某一时刻，TLB、页表和L1 Data Cache中部分内容如下所示：
...
关键在于虚拟地址，既用于页表有用于 TLB
虚拟地址 16 位，页大小 128B，页内偏移 7 位，故页号 16-7=9 位
TLB 四路组相联，16 个页表项，总共 4 组，故块号 2 位，TLB 其实不需要页内偏移（参见 TLB 图），所以块内偏移和页内偏移保持一致，占 7 位，剩下 16-7-2=7 位是标记位
![|650](Images/Pasted%20image%2020241217205233.png)
2.物理地址中哪几位表示物理页号？哪几位表示页内偏移量？
物理地址 12 位，低 7 位是页内偏移，高 5 位是物理页号
3.主存（物理）地址如何划分成标记字段、行索引字段和块内地址字段？
Cache 采用直接映射，块大小为 4B，所以块内偏移占 2 位；共 16 行，所以块号 4 位；剩余 12-2-4=6 位作标记位
4.CPU从地址067AH中取出的值为多少？说明CPU读取地址067AH中内容的过程
此时取出的地址067AH是虚拟地址，根据题意与分析，先在TLB中查快表：
067AH二进制表示为：<font color="#4bacc6">0000 011</font><font color="#d83931">0 0</font><font color="#4bacc6">111 1010</font>，中间2位是00，故应查TLB的第0组；又标记位是：03H，于是找到第0组的第0行，此时它的有效位是0，无效，所以继续查找主存中的页表；高7位虚拟页号是0CH，对应的页框号是19H且有效位是1，这样虚拟地址对067AH对应的物理地址是1100 11<font color="#d83931">11 10</font>10
欲访问物理地址1100 1111 1010的主存单元，先查cache，根据3的分析，物理地址中间4位是：1110，查找到cahce的第15行，有效位是1，且标记位是33H，二进制表示为110011 对应物理地址的高6位。又物理地址低2位是10，则对应的是第 2个字节，内容是4A。所以，从067AH中取出的内容是4AH
# IO
I/O 接口(I/O 控制器)的功能：
1. 数据缓冲
   实现主机和外设工作速度的匹配
...
5. 异步 I/O：不会导致请求进程阻塞
![|500](Images/Pasted%20image%2020241217181029.png)
**数据传送控制方式**
## 程序直接控制
...
- 地址、数据传送重叠
（单选）增加同步总线带宽的手段有很多，但以下哪种方式不能提高总线带宽？
A. 采用信号线复用技术
B. 增加总线带宽
C. 采用突发传送方式
D. 提高总线时钟频率
信号线复用技术：同一组信号线在不同的时刻传输不同的信息，例如地址线和数据线复用，表示总线事务开始时，这组信号用来传输地址，在数据传输阶段这组信号线又用来传输数据。但如果存储器写总线事务，采用复用，开始时 CPU 不能把地址和数据同时发送到总线上，必须先发送地址，再发送数据，这样会延长该总线事务的时间，无法提高总线带宽，选 A
（填空）某同步总线的时钟频率为 100Mhz，宽度为 32 位，地址/数据线复用，每传输一个地址或数据占用一个时钟周期。若该总线支持突发传输方式，则一次“主存写”总线事务传输 128 位数据所需要的时间至少是？
总线频率 100Mhz，时钟周期为 10ns，总线宽度与存储字长都是 32 位，故每次传送一个 32 位存储字。若 128 位数据地址连续，则传送首地址一个周期 10ns，传输数据 128/32=4 个周期 40ns，共计 50ns
## 习题
