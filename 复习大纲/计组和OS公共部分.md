...
## 习题
假定计算机系统有一个容量为 32KB 的主存，，且有一个联 Cache，，主存和且有一个 4KB 的 4 路组相主存和 Cache 之间的数据交换块的大小为 64B。。假定假定 Cache 开始为空，，处理器顺序地从存储单元处理器顺序地从存储单元 0、1、…、4351 中取一个 Byte，一共重复 10 次次。。。设设 Cache 比主存快 10 倍倍。。。采用采用 LRU 算法。。试分析 Cache 的结构和主存地址的划分。。说明采用试分析说明采用 Cache 后速度提高了多少假定计算机系统有一个容量为 32KB 的主存，，且有一个联 Cache，，主存和且有一个 4KB 的 4 路组相主存和 Cache 之间的数据交换块的大小为 64B。。假定假定 Cache 开始为空，，处理器顺序地从存储单元处理器顺序地从存储单元 0、1、…、4351 中取一个 Byte，一共重复 10 次次。。。设设 Cache 比主存快 10 倍倍。。。采用采用 LRU 算法。。试分析 Cache 的结构和主存地址的划分。。说明采用试分析说明采用 Cache 后速度提高了多少
# 内存管理
