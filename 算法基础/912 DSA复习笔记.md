...
tab: 4
**当前元素（5）小于栈顶元素（7），开始计算栈顶元素所在矩形面积，如图，由于我们是用单调栈维护，所以7在栈内的下一个元素（6）一定小于7，而6对应的下标（0）也就是高度为7的矩形的左边界；注意到弹栈操作只会发生在当前元素（5）小于栈顶元素（7）时，所以我们可以知道高度为7的矩形的右边界就是当前元素（5）的下标（2）**
