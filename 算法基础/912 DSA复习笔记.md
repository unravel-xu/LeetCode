...
求割点和求点双可以同步进行，在求割点的过程中，我们发现 $low[6] \geq dfn[3]$，得出 3 是一个割点，此时递归调用栈中存储的是 {1，**3，5，2，4，6**}，正好包含点双分量，由此我们可以用栈来保存，一旦发现某个节点是割点，我们就可以将栈弹出到割点位置。但是我们需要注意孤立单点也构成一个点双，以及栈的 `pop()` 操作
