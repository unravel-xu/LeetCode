...
## 12-D 锦标赛排序
### 胜者树
起因：小根堆每次取出堆顶元素后需要下滤调整，下滤过程中每层为选出最小值都要进行两次比较，胜者树可以减少比较次数：
````tab
tab:1
**选择一个最小的数字为胜利者，第一次将各数组里面的第一个元素都放进去，两两比较得出最终胜者为1**

tab:2
**将第一轮胜者所在数组下一个元素放进去，此时图中蓝框部分可以不再比较，比较得到所有数组的第2小元素是2**
