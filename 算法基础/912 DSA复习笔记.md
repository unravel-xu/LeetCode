...
从低位到高位，依次以各域为序做一趟桶排序，由于**桶排序的稳定性**，第 i 趟中相同字段的大小取决于最低的 i-1 个字段
时间复杂度：等价于各趟桶排序所需时间之和= $\sum^{t}_{i=1} O(n+2m_{i}) = O(t*(n+\max\{m_{1},\dots ,m_{t}\}))$，当 $m=O(n)$ 且 t 可以视作常数时（），就是 O(n)
## 12-C 堆排序
![](Images/heapSort.gif)
向量选择排序，选择最大/最小时借用堆
