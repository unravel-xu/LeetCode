...
* parent 数组，数组中的值表示该顶点的父节点编号，用于更新 low 数组时排除父节点
````tab
tab: 1
...
如图，B 节点的孩子节点是 J，C；$low[J] = 2 \geq 2 = dfn[B]$，$low[C] = 2 \geq 2 = dfn[B]$，所以 B 节点就是一个割点
节点 C 的孩子节点是 D；$low[D] = 2 < 3 = dfn[C]$，所以 C 节点不是一个割点
# 排序
## 02-E 向量-冒泡排序
```c++
// 跳跃版冒泡排序
