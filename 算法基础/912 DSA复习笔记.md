...
第 k 层的节点期望数 $E(|S_{k}|) = \frac{n}{2^k}$，节点总数的期望是 $$E\left( \sum_{k}|S_{k}| \right) = \sum_{k}E\left(|S_{k}| \right) = \sum_{k} \frac{n}{2^k} < 2n = O(n)$$
跳转表期望高度和纵向跳转次数：$O(\log n)$，不过，跳跃表并不提供对最坏情形的性能保证，因为并不能排除非常小的概率下会生成一个极不平衡的跳跃表。那时，查找一个元素的时间将退化为 
O
(
n
)
...
转载请注明原文链接: https://writings.sh/post/data-structure-skiplist#%E9%9A%8F%E6%9C%BA%E5%8C%96
# 优先级队列
## 12-B 完全二叉堆
