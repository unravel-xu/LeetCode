...
tab: 12
![|500](Images/Pasted%20image%2020240812205522.png)
````
**割点**：判断顶点U是否为割点，用U顶点的dfn值和它<font color="#d83931">所有的孩子</font>顶点的low值进行比较，如果存在至少一个孩子顶点V满足 $low[v] \geq dfn[u]$，就说明顶点V访问顶点U的祖先顶点，必须通过顶点U，而不存在顶点V到顶点U祖先顶点的其它路径，所以顶点U就是一个割点。对于没有孩子顶点的顶点（叶节点），显然不会是割点。
...
### 07-F Floyd-Warshall 算法
利用动态规划寻找给定的加权图中**多源点之间**最短路径的算法
思路：从 `i` 到 `j` 的最短路径有两种情况：
1. 是从 i 直接到 j
2. 是从 i 经过若干个中间节点到达 j，对于任意 i ，j 和**任意中间节点** `k`，判断 $d(i)(k) + d(k)(j) < d(i)(j)$ 是否成立，如果成立，则更新 $d(i)(j) = d(i)(k) + d(k)(j)$
定义：
$d[k][i][j]$：只允许经过节点 1~k 时，节点 i 到节点 j 的最短路径长度，所以 $d[n][i][j]$ 就是节点 i 到节点 j 的最短路长度
$d[k-1][i][j]$：i 到 j 的路径不经过点 k
$d[k-1][i][k] + d[k-1][k][j]$：i 到 j 的路径经过点 k
由此得到状态转移方程：
$$d[k][i][j] = min(d[k-1][i][j], d[k-1][i][k]+d[k-1][k][j])$$
**初始条件**：$d[0][i][j] = w(i, j)$，在不使用任何点的情况下，两点之间最短路径的长度就是两点之间边的权值（若两点之间没有边，则权值为正无穷）
![|300](Images/Pasted%20image%2020240919161506.jpg)
````tab
tab:1
**初始时，任意两点之间不允许经过第三个点，两点最短距离就是初始距离**
![](Images/Pasted%20image%2020240919161650.png)
tab:2
**只允许经过顶点1的情况下，3号->2号，4号->2号，4号->3号路程都变短了，3->2 = min((3->1 + 1->2), 3->2) = min((7+2), $\infty$)=9**
![](Images/Pasted%20image%2020240919161633.png)
tab:3
**只允许经过1号和2号顶点的情况下，1号->3号，4->3号路程都变短了，4->3 = min((4->2 + 2->3), 上一轮中的4->3) = min((7+3), 11) = 10**
![](Images/Pasted%20image%2020240919162257.png)
tab:4
**允许通过1，2，3号顶点：**
![](Images/Pasted%20image%2020240919162336.png)
tab:5
**允许通过所有顶点：**
![](Images/Pasted%20image%2020240919162403.png)
````
时间复杂度：$O(n^3)$
空间复杂度：$O(n^2)$
## 最小生成树
...
### 07-E Kruskal 算法
查询两点是否连通和连接两点可以使用并查集维护，若选定边的两端点在同一集合中，则加入此边必定成环
![](https://www.bilibili.com/video/BV1Eb41177d1/)
时间复杂度：记 $\mid Edges \mid = m$，如果使用 $O(m\log m)$ 的排序算法，并使用 $O(m\alpha(m,n))$ 或 $O(m\log n)$ 的并查集，可以得到 $O(m\log m)$，适用于稀疏图
# 二叉搜索树
$$左后代\leq任一节点\leq右后代$$
...
# 词典
## 11-G 跳转表
![](Images/Pasted%20image%2020240913220012.jpg)
每一水平列表称为一层（level），层次不同的节点可能沿纵向组成塔
### 查找
![](Images/Pasted%20image%2020240913220141.jpg)
### 复杂度
随机化：当插入元素时，新节点成为 k 层的概率是 $\frac{1}{2^k}$，即新节点插入在第 0 层后，开始“抛硬币”，正面朝上则增加一层并继续“抛”，直到出现反面。

第 k 层的节点期望数 $E(|S_{k}|) = \frac{n}{2^k}$，节点总数的期望是 $$E\left( \sum_{k}|S_{k}| \right) = \sum_{k}E\left(|S_{k}| \right) = \sum_{k} \frac{n}{2^k} < 2n = O(n)$$ ![](Images/Pasted%20image%2020240913221920.jpg)跳转表查找过程中横向跳转和纵向跳转次数：$O(\log n)$，最坏情况下时间将退化为 $O(n)$，总体期望查找时间：$O(\log n)$
单个词条的期望塔高是 2，整体期望高度是 $\Theta(\log n)$
<font color="#d83931">同层</font>横向跳转期望次数：2 次
### 插入
1. 找到各层的插入位置，例如插入元素 20，`last[k]` 表示找到的第 k 层中的前驱节点
![](Images/Pasted%20image%2020240913222450.jpg)
2. 根据随机函数决定新节点的阶
![](Images/Pasted%20image%2020240913222701.jpg)
3. 各层执行插入操作
![](Images/Pasted%20image%2020240913222807.jpg)
### 删除
1. 找到各层的前驱节点和要删除的节点，如下：删除 22
![](Images/Pasted%20image%2020240913222906.jpg)
2. 各层执行删除，注意 2 层及以上不是前驱，因为 22 只有 1 层
![](Images/Pasted%20image%2020240913223000.jpg)
参考：[跳跃表的构造思路和基本操作](https://writings.sh/post/data-structure-skiplist)
# 优先级队列
## 12-B 完全二叉堆
...
## 12-F 左式堆
左式堆的精髓在于**合并**
![|300](Images/Pasted%20image%2020240823214926.png)
左式堆基于树而不是像完全二叉堆基于向量
定义：$npl(NULL) = 0$
$npl(x) = 1 + \min\{npl(x\to lc),npl(x\to rc)\}$，也就是节点 x 到外部节点的**最近距离** / 以 x 为根的**最大满子树高度**
图中 npl(1) = 3，是因为沿箭头路径长度为 3
左式堆：对堆中任意节点 x ，都满足 $npl(x\to lc)\geq npl(x\to rc)$，所以左式堆中 $npl(x) = 1 + npl(x\to rc)$，也由此可知左式堆的子堆必是左式堆
极端情况：
![|300](Images/Pasted%20image%2020240919172551.png)
右侧链长为 d 的左式堆，**至少**包含
- $2^d-1$ 个内部节点（以 d 为高的满树）
- $2^{d+1}-1$ 个节点（外部节点都位于 d+1 层）
### 12-F3 合并
...
## 12-XA 优先级搜索树
![|400](Images/Pasted%20image%2020240919190243.png)
优先搜索树(Priority Search Tree, PST)是一种二叉搜索树，存储二维坐标点(x,y)，可以高效查询区间 `[min_key, max_key]` 和区间 `[-∞, max_priority]` 中的值。PST同时具备一维区域树和堆的特性：对于任何节点，它的左子树的 x 坐标总是不大于右子树的 x 坐标值。即从 x 坐标来看，PST 具有一维区域树的特征；而且任何子树根节点的 y 坐标值都是该子树的最小值（类似胜者树），这又使得 PST 具有最小堆的特点。
* 查询优先级不超过 2 的节点，从根节点开始，我们发现根节点 y=3>2，而根节点的 y 记录了全局最小优先级，所以直接返回失败
* 查询优先级不超过 5 且 $key \in [7, 20]$，从根节点开始，根节点 x=23>20，所以我们转到左子树查找；根节点 $x=15 \in [7,20]$，将此节点加入到 result 中，并递归转到左子树查找，左子树递归查找结束后再转到此节点的右子树查找
# 串
## 13-C KMP
### 未改进
构造 `next[]` 表：
...
以计算 I 的 next 值为例，由于我们已经知道 I 前一个字符 M 的 next 值为 3，说明在 `[0, 4]` 子串中，最大自匹配前后缀长度为 3（"MAM"），此时我们比较 `p[next[cur]] = p[3] = "A"` 是否和 `p[cur] = "M"` 相同，发现不相同，所以我们要找"MAM"子串中最大自匹配前后缀，也就是 `p[next[next[cur]]] = p[next[3]] = p[1] = "A"` 如此迭代比较
```c++
...
注意，next 表值表示：当前字符的真前缀中最大自匹配前后缀长度，且自匹配的前后缀下一个值不能相同。如下图，当 cur = 5 -> "M"，`改进next[cur]=3=prefix`，$pattern[prefix] = pattern[3] = A \neq M = pattern[cur] = pattern[5]$；再如 cur = 2 -> "M"，`改进的next[cur]=-1`：`pattern[2]` 的真前缀是"MA"，没有匹配的前后缀，且在 `pattern[2]` 处发生失配，意味着字符串此时为"MA(!M)"，所以下一次匹配不能再将模式串首的“M”对齐，而是直接跳到下一个位置，同理知 `改进的next[3]=0`，因为此时发生失配的字符虽然不等于“A”但可能等于模式串首的“M”
==改进的 next 表第 1 项不一定是 0，也可能是-1==，例如"AAB"，`[-1,-1,1]`
...
tab:1
**改进的next表第0项一定是-1**
![](Images/Pasted%20image%2020240919213744.png)
tab:2
**在B处失配，所以字符串为"A(!B)"，下一次将模式串首"A"对齐失配位置**
![](Images/Pasted%20image%2020240919213847.png)
tab:3
**在A处失配，所以字符串为"ABCD(!A)"，此时模式串首"A"和失配位置"A"相同，下一次直接跳过当前失配位置**
![](Images/Pasted%20image%2020240919214115.png)
tab:4
**在B处失配，所以字符串为"ABCDA(!B)"，用串首A匹配，0对应A的下标**
![](Images/Pasted%20image%2020240919222135.png)
tab:5
![](Images/Pasted%20image%2020240919222655.png)
tab:6
**用"ABCDA"中的"A"和"ABCDG"中的"G"对齐，填用于对齐的"A"下标4**
![](Images/Pasted%20image%2020240919223052.png)
tab:7
![](Images/Pasted%20image%2020240919223536.png)
tab:8
![](Images/Pasted%20image%2020240919223617.png)
````
## 习题
1. 模式串 HHFBHHFHHFBSHF0
>[!note]-
>![](Images/Pasted%20image%2020241204210821.png)

2. 模式串 RVMMOREOTRVMP
>[!note]-
>![](Images/Pasted%20image%2020241204211936.png)

3. 模式串 BABABBBBABABBABBA
>[!note]-
>![](Images/Pasted%20image%2020241204212110.png)
## 13-D BM
### 13-D2 BC 策略
...
```
![](Images/Pasted%20image%2020241130160112.png)
![](Images/Pasted%20image%2020241130160139.png)
### 13-E1 GS 策略
...
````tab
tab: 1
**gs[14] = 1**: 当在"E"处失配时，前一位"C"!="E"，将字符串后移一位继续匹配
...
情况二：例如 `gs[10] = 6`
![](Images/Pasted%20image%2020240920154146.png)
...
## 习题 3-9 向量-选择排序
稳定性：稳定(自后向前逆向扫描，只有遇到严格更大的元素时，才更新最大元素的记录)
## 03-E 列表-选择排序
````tab
tab: 1
...
> [!attention] 
> 注意 912 中桶排序和上述不同，其借助散列表，将待排序整数依次映射到散列表中
> 
> 给定 $[0, M)$ 内的 $n$ 个互异整数 $n\leq M$
> ![](Images/Pasted%20image%2020240913173758.png)
> 顺序遍历一次散列表并依次输出非空桶中存放的关键码
> 
> 空间复杂度：散列表所占空间 $O(M)$
> 
> 时间复杂度：散列表的创建和初始化 $O(M)$，将所有关键码插入散列表 $O(n)$，依次读出非空桶中的关键码 $O(M)$，总体运行时间 $O(n+M)$
> 
> 一般情况下，待排序整数可能存在重复，采用独立链串接
> ![](Images/Pasted%20image%2020240913175205.png)
> 空间复杂度：$O(M)$
> 
> 时间复杂度：$O(n+M)$
> 
> 稳定性：稳定
> 
> 当 $n\gg M$ 时，时间复杂度 $O(n+M) = O(\max(n,M)) = O(n)$

[164. 最大间距](https://leetcode.cn/problems/maximum-gap/)
## 11-E 基数排序
![](Images/Pasted%20image%2020240913183748.png)
预处理：对于一个 d 进制的数字 x ，要获取其第 k 位 $x_{k}$：$x_{k} = \left\lfloor \frac{x}{d^{k-1}} \right\rfloor\mod{d}$，所以 n 个数，总处理时间为 $O(n*t)$ （数字 x 有 t 位）

从低位到高位（保证高位排序结果可以覆盖低位），依次以各域为序做一趟桶排序，由于**桶排序的稳定性**，第 i 趟中相同字段的大小取决于最低的 i-1 个字段

时间复杂度：预处理时间+各趟桶排序所需时间之和= $O(t*n)+\sum^{t}_{i=1} O(n+2m_{i}) = O(t*(n+\max\{m_{1},\dots ,m_{t}\}))$，当 $m=O(n)$ 且 t 可以视作常数时（数据位数不太大，数据范围和数据量同阶）-> O(n)
空间复杂度：$O(n+M)$
稳定性：当桶排序稳定时，基数排序也稳定；当桶排序不稳定时，基数排序无法保证得到正确的排序结果。
## 11-F 计数排序 0
对来自 $[0,M)$ 范围内的 n 个整数进行排序
![|500](Images/Pasted%20image%2020241130105623.png)
![](Images/Pasted%20image%2020241130110433.png)
最终输出 $s=[2a,2b,2c,3,5a,5b,8,9a,9b]$
时间复杂度：$O(n+M)$
空间复杂度：$O(n+M)$
稳定性：倒序遍历可以避免改变相等元素之间的相对位置，从而实现稳定排序。实际上，正序遍历也可以得到正确的排序结果，但结果是非稳定的。
局限性：**计数排序只适用于非负整数**
## 12-C 堆排序
![](Images/heapSort.gif)
向量选择排序，选择最大/最小时借用堆
...
````tab
tab:1
**选择一个最小的数字为胜利者，第一次将各数组的第一个元素放入，两两比较得出最终胜者为1**
![](Images/Pasted%20image%2020240919150816.png)
tab:2
**将第一轮胜者所在数组下一个元素放进去，比较得第二小元素是2**
![](Images/Pasted%20image%2020240919151230.png)
tab:3
![](Images/Pasted%20image%2020240919151832.png)
````
### 败者树
每次比较，记录败者到父节点，并推举胜者进一步比较
````tab
tab:1
![](Images/Pasted%20image%2020240919113153.png)
tab:2
![](Images/Pasted%20image%2020240919113929.png)
tab:3
![](Images/Pasted%20image%2020240919114143.png)
tab:4
![](Images/Pasted%20image%2020240919114620.png)
tab:5
![](Images/Pasted%20image%2020240919145311.png)
tab:6
![](Images/Pasted%20image%2020240919145652.png)
````
* 堆每次取出最小值之后，把最后一个数放到堆顶，调整堆的时候，每次都要选出父节点的两个孩子节点的最小值，然后再用孩子节点的最小值和父节点进行比较，所以每调整一层需要比较两次。
* 胜者树每次比较只用跟自己的兄弟节点进行比较就好，所以用胜者树可以比堆少一半的比较次数。但胜者树在节点上升的时候需要先获得父节点，再获得兄弟节点，然后再比较。
* 败者树，每个新元素上升时，只需要获得父节点并比较即可，且在与父节点比较失败才需要写入，而胜者树成功就要写入。所以总的来说，比胜者树减少了访存的时间。
败者树插入、删除(相当于插入 $+\infty$)时间复杂度相较于胜者树更优
## 14-A 快速排序
...
快速排序的递归次数与元素的初始排列有关，而与划分后对分区的处理顺序无关：
![|400](Images/Pasted%20image%2020240920190100.png)
## 14-B 选取特定数
### 众数
...
时间复杂度：$O(n)$；空间复杂度：$O(1$)
【习题 12-2】返回值在向量中出现的次数最少可能是多少？
![](Images/Pasted%20image%2020241130165117.png)
### 归并向量的中位数
...
【习题 12-10】在 $S1\cup S2$ 中选取第 k 个元素
![|500](Images/Pasted%20image%2020241130165845.png)
...
5. 部分的快速排序（快速选择算法），每次划分之后判断第k个数在左右哪个部分，然后递归对应的部分。<font color="#d83931">平均时间</font>复杂度为$O(n)$。但最坏情况下复杂度为$O(n^2)$
...
BFPRT 算法步骤如下：
	...
	3. 以该中位数 M 作为pivot，对序列进行Partition，原序列将被分为 L/E/G三个部分 O(n)：![|450](Images/Pasted%20image%2020240901202728.png)
