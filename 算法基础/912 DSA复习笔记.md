...
![](Images/Pasted%20image%2020240828114124.png)
情况二，此时 `MS[m-hi+j-1]` 长度过长，超过了红框， $j-lo<ss[m-hi+j-1]$
但是我们可以确定蓝色部分（在红框内）一定是匹配的：`P(lo, j] = P[m-hi+lo, m-hi+j)`，接下来只需比较玫红色部分和青色部分
![](Images/Pasted%20image%2020240828142847.png)
