...
第 k 层的节点期望数 $E(|S_{k}|) = \frac{n}{2^k}$，节点总数的期望是 $$E\left( \sum_{k}|S_{k}| \right) = \sum_{k}E\left(|S_{k}| \right) = \sum_{k} \frac{n}{2^k} < 2n = O(n)$$![](Images/Pasted%20image%2020240913221920.jpg)（书 P 254）跳转表期望高度和纵向跳转次数：$O(\log n)$，最坏情况下时间将退化为 $O(n)$

同层横向跳转期望次数：2 次，整个查找过程中横向跳转的期望次数正比于跳转表期望高度：$O(\log n)$
### 插入
...
* 败者树，每个新元素上升时，只需要获得父节点并比较即可。所以总的来说，比胜者树减少了访存的时间。
## 14-A 快速排序
...
```
时间复杂度：$O(\log(\min\{{n_{1},n_{2}}\}))$、[4. 寻找两个正序数组的中位数](https://leetcode.cn/problems/median-of-two-sorted-arrays/) [官方题解](https://www.bilibili.com/video/BV1Xv411z76J/?spm_id_from=333.337.search-card.all.click&vd_source=b404977d131639c9b47b18fe2cc16c2c)给了另一种 $O(\log(\min\{{n_{1},n_{2}}\}))$ 的算法
### 第 K 大元素选择算法
