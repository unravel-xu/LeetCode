...
预处理：将所有元素转换为 n 进制形式，假设待转换的数字在10 进制下为 x，进制转换时间为 $O(\log_{n}x)$
从低位到高位（保证高位排序结果可以覆盖低位），依次以各域为序做一趟桶排序，由于**桶排序的稳定性**，第 i 趟中相同字段的大小取决于最低的 i-1 个字段
时间复杂度：预处理时间+各趟桶排序所需时间之和= $\sum^{t}_{i=1} O(n+2m_{i}) = O(t*(n+\max\{m_{1},\dots ,m_{t}\}))$，当 $m=O(n)$ 且 t 可以视作常数时（数据位数不太大，数据范围和数据量同阶）-> O(n)
