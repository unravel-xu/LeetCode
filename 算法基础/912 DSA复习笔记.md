...
- 先考虑两向量等长情况：
![](Images/Pasted%20image%2020240901101648.png)
如图，有序向量 $s_{1}，s_{2}$ 长度相等且为奇数
当 $m_{1} = m_{2}$ 时，因为 $L_{1} \leq m_{1}, L_{2} \leq m_{2},m_{1} \leq R_{1}, m_{2} \leq R_{2}$ 我们可以知道归并后的有序向量必然形如：$\{L_{1} \cup L_{2} \}$ $m_{1}(m_{2})$ 就是归并后的中位数， 
### 第 K 大元素选择算法
