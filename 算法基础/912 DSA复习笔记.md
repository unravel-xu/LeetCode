...
```c++
void insert(int x, vector<int> &BinHeap)
{
    BinHeap.push_back(x);
    // 记录插入元素位置
    ...
    while (cur)
    {
		// 注意cur的父亲位置是(cur-1) / 2
		// 小根堆
        if ((cur - 1) >> 1 >= 0 && BinHeap[cur] < BinHeap[(cur - 1) >> 1])
        {
            swap(BinHeap[cur], BinHeap[(cur - 1) >> 1]);
            cur = (cur - 1) >> 1;
        }
        else
            break;
...
```c++
void delMin(vector<int> &BinHeap)
{
	// 交换
    ...
    int cur = 0;
    // 将cur与其最小的孩子节点交换
    while (cur < BinHeap.size())
    {
        int min = cur;
        if (cur * 2 + 1 < BinHeap.size() && BinHeap[min] > BinHeap[cur * 2 + 1])
            min = cur * 2 + 1;
        if (cur * 2 + 2 < BinHeap.size() && BinHeap[min] > BinHeap[cur * 2 + 2])
            min = cur * 2 + 2;
        // 如果cur比两个孩子都小，则堆序性满足
        if (min == cur)
            break;
        swap(BinHeap[cur], BinHeap[min]);
        cur = min;
    }
}
```
